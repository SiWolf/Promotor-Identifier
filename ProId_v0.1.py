...
def final_results(fastaList):
    #Gibt Ergebnisse der bisherigen Analysen in einer TSV-Datei aus
    final_path = os.path.join(genomeDir, "all_promoters_results.tsv")
...
def analyseBlastResult(genome, blastfile):
    #Ließt eine BLAST-Ergebnisdatei aus und parst die Ergebnisse in eine TSV und die Promoter.txt
    blast_results_path = os.path.join(genomeDir, genome + "_parse_results.tsv")
...
def regular_expression_search():
    ...
    regular_expression_results_path = os.path.join(genomeDir, "all_promoters_regular_expression_search.tsv")
